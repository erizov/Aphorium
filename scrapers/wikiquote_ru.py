...
class WikiQuoteRuScraper(BaseScraper):
    """Scraper for Russian WikiQuote."""

    def __init__(self):
        """Initialize Russian WikiQuote scraper."""
        super().__init__(
            base_url=settings.wikiquote_ru_base_url,
    ...
    def scrape_author_page(self, author_name: str) -> dict:
        ...
        # WikiQuote RU typically has sections for different works
        headings = soup.find_all(["h2", "h3"])

        for heading in headings:
            # Skip non-content headings
            heading_id = heading.get("id", "")
            if "mw-toc" in heading_id or "Ссылки" in heading.get_text():
                continue
    ...
    def _is_valid_source_title(self, title: str) -> bool:
        ...
        nav_keywords = [
            "содержание", "навигация", "ссылки", "внешние ссылки",
            "см. также", "примечания", "источники", "библиография"
        ]
        ...
        # Should be reasonably long (not just "Часть I" or "Глава 1")
        if len(title) < 5:
            return False
        
        # Skip if it matches reference patterns
        import re
        if re.match(r'^Часть\s+[IVX]+(?:\s*[:\-]|\s*$)', title):
            return False
        if re.match(r'^Глава\s+\d+(?:\s*[:\-]|\s*$)', title):
            return False
        if re.match(r'^Раздел\s+\d+(?:\s*[:\-]|\s*$)', title):
            return False
